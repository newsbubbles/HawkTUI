...
use hawktui::core::{
    ...
    events::{Action, AgentEvent, Event, map_key_to_action},
    keybindings::{KeyBindings, parse_action_string, parse_key_string},
    state::{
        AppMode, AppState, ConnectionStatus, InputState, LayoutMode, Message, MessageRole, Overlay,
        Panel, StatusInfo,
    },
};
...
fn test_layout_mode_from_str() {
    ...
    assert_eq!(
        LayoutMode::from_str("command-center"),
...
fn test_commands_defined() {
    ...
    let expected = [
        "help", "clear", "exit", "model", "provider", "session", "theme", "layout", "export",
        "context", "tools", "system", "branch", "compact", "tokens", "cost", "vim",
    ];
...
fn test_parse_action_string() {
    ...
    assert_eq!(
        parse_action_string("toggle_layout"),
...
use hawktui::ui::widgets::{SPINNER_FRAMES, Spinner, next_frame};
...
#[tokio::test]
async fn test_pi_bridge_creation() {
    let bridge = PiBridge::new(None, None);
    assert_eq!(bridge.model().await, "claude-sonnet-4-20250514");
    assert_eq!(bridge.provider().await, "anthropic");
    assert!(!bridge.is_connected());
}

#[tokio::test]
async fn test_pi_bridge_custom_model() {
    let bridge = PiBridge::new(Some("gpt-4o".to_string()), Some("openai".to_string()));
    assert_eq!(bridge.model().await, "gpt-4o");
    assert_eq!(bridge.provider().await, "openai");
}

#[tokio::test]
async fn test_pi_bridge_set_model() {
    let mut bridge = PiBridge::new(None, None);
    bridge
        .set_model("anthropic", "claude-3-5-sonnet-20241022")
        .await
        .unwrap();
    assert_eq!(bridge.model().await, "claude-3-5-sonnet-20241022");

    // set_provider() removed - use set_model() instead
    bridge.set_model("openai", "gpt-4o").await.unwrap();
    assert_eq!(bridge.provider().await, "openai");
}
